...
def process_feature_importance(model, x_train_with_noise):
    """
    ...
    Returns:
    importance_df (pd.DataFrame): Full importance breakdown.
    useful_features (list): Features with gain higher than noise.
    useless_features (list): Features with 0 gain or lower than noise.
    """
    ...
    # Useful: Gain strictly greater than noise
    useful_features = importance_df[importance_df['gain'] > noise_threshold]['feature'].tolist()
...
def processing_from_csv(csv_train, csv_val, csv_test, metadata_path, steps=10, masked_flag=False, score_type="auc",
    ...
    while current_step < steps:
        ...
        if error:
            ...
            for i, (name, score, _, _, _) in enumerate(candidates):
